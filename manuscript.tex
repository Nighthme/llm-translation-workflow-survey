...
\title{From Neural Machine Translation to LLM-Augmented Translation Workflows: A Workflow-Oriented Scoping Survey}
...
This paper therefore treats LLM-augmented translation as a socio-technical NLP problem. The technical system includes models, prompts, retrieval resources, decoding settings, benchmarks, and metrics. The social and professional system includes translators, reviewers, clients, language communities, institutional confidentiality requirements, domain specialists, and end users. A survey that ignores either side will miss the central question: under what workflow conditions can LLMs be trusted to support translation?
...
The formal search protocol was consolidated for the present draft on 9 June 2026, and the public repository records the search strings, screening decisions, coding schema, metadata-cleanup notes, and PRISMA-style counts. Because this is a scoping survey of a fast-moving field rather than a meta-analysis, the search combined database-style fielded strings with venue-specific and citation-chaining discovery. The complete query blocks, Scopus and Web of Science fielded strings, ACL Anthology variants, human-workflow search line, and gap-fill search lines are archived in the project search-strategy file. The screening and extraction table records the source database or discovery route, publication status, inclusion decision, workflow stage, and reproducibility notes for each record.
...
\node[box,right=0.95cm of screen,fill=softred,text width=4.25cm] (excluded) {Not in full extraction\\background/calibration records $n=2$; excluded/unstable records $n=5$; metadata-caution record $n=1$};
...
\end{tikzpicture}
\caption{PRISMA-style flow diagram for the final extraction corpus. The side branch separates records retained for background or journal calibration, records excluded after screening, and one metadata-caution record left outside full extraction. Counts reflect the project search, deduplication, metadata-cleanup, and screening logs available at the time of manuscript preparation.}
\end{figure}
...
Five challenges stand out. First, professional deployment evidence remains thin. Many studies use benchmark datasets, short texts, or laboratory tasks; fewer examine real CAT-tool workflows, client constraints, translation-memory governance, confidentiality, revision cycles, or longitudinal adoption. This matters because a system that looks useful in a short experiment may change work practices over months.

Second, privacy and confidentiality are underdeveloped in translation-specific research. Translators frequently handle unpublished manuscripts, contracts, medical records, immigration documents, litigation materials, internal corporate reports, and culturally sensitive community texts. Future work should specify whether data can be sent to external APIs, whether local models are required, and how translation memories are protected.

Third, human-centred outcomes are not yet standard. Cognitive load, trust, effort, productivity, and expertise should not be occasional add-ons. A translation workflow that improves COMET but increases translator monitoring burden may not be a better workflow.
...
Fifth, evaluation loops can become circular. If LLMs translate, evaluate, explain, and post-edit, then model-family biases may be reinforced. The field also needs more negative evidence about when LLMs fail, increase effort, fail to transfer across prompts, or produce suggestions that human reviewers reject.

\subsection{Limitations}
This survey has several limitations. First, it is a scoping survey rather than a systematic review or meta-analysis. The goal is to map workflow roles, evidence clusters, and gaps rather than estimate pooled effects. The corpus is therefore deliberately focused on workflow-relevant records and does not claim to exhaust all MT or LLM translation research from 2017 to 2026.

Second, screening and coding were conducted by a single author. This is a limitation because borderline inclusion decisions and thematic coding can be affected by reviewer judgement. The study mitigates this risk by using a structured extraction table, documenting search strings and inclusion decisions, revisiting uncertain records after metadata cleanup, and releasing the screening and coding files through the public repository.

Third, the literature is changing quickly. Some records appear first as preprints, workshop papers, ahead-of-print articles, or shared-task system descriptions. Metadata can change after indexing, and proprietary LLM behaviour can change with model updates. The audit trail and DOI checks reduce this risk, but the corpus should be understood as a time-stamped map of the field rather than a permanently closed bibliography.

Fourth, the evidence base is uneven across workflow areas. Post-editing and LLM-based evaluation are relatively well represented, while professional deployment, privacy, longitudinal trust, high-stakes domain workflows, and low-resource community validation remain thinner. The absence of evidence in these areas should not be interpreted as evidence that the corresponding risks are unimportant.
